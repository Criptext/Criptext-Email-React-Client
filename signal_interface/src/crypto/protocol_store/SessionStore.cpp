...
int session_store_load_session(signal_buffer **record, signal_buffer **user_record, const signal_protocol_address *address, void *user_data)
{
    ...
    try {
        sessionRecord = CriptextDB::getSessionRecord(account->getDB(), account->password, recipientId, deviceId);
    } catch (exception& e) {
...
int session_store_get_sub_device_sessions(signal_int_list **sessions, const char *name, size_t name_len, void *user_data)
{
    ...
    vector<CriptextDB::SessionRecord> sessionRecords = CriptextDB::getSessionRecords(account->getDB(), account->password, recipientId);
...
{   
    ...
    bool success = CriptextDB::createSessionRecord(account->getDB(), account->password, recipientId, deviceId, recordBase64, len);
    return success ? 1 : 0;
}
...
{
    CriptextDB::Account *account = (CriptextDB::Account*)user_data;
    //string dbPath(account->getDB());

    std::string recipientId = std::string(address->name);
    int deviceId = address->device_id;

    try {
        CriptextDB::getSessionRecord(account->getDB(), account->password, recipientId, deviceId);
    } catch (exception& e) {
        return 0;
    }
...
int session_store_delete_session(const signal_protocol_address *address, void *user_data)
{
    ...
    std::string recipientId = std::string(address->name);
    int deviceId = address->device_id;
    bool success = CriptextDB::deleteSessionRecord(account->getDB(), account->password, recipientId, deviceId);
...
int session_store_delete_all_sessions(const char *name, size_t name_len, void *user_data)
{
    ...
    std::string recipientId = std::string(name, name_len);
    bool success = CriptextDB::deleteSessionRecords(account->getDB(), account->password, recipientId);
